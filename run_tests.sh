...
sudo ./run.sh ../masim /sys/fs/cgroup/ 1350M Y Y N \
	./compressible_data ./compressible_data /dev/urandom 120 | \
	tee "$log_dir/1-2"

echo
echo "zswap enabled with keeping incompressible pages, 10 % memory pressure"
sudo ./run.sh ../masim /sys/fs/cgroup/ 1350M Y Y Y \
	./compressible_data ./compressible_data /dev/urandom 120 | \
	tee "$log_dir/1-3"
...
sudo ./run.sh ../masim /sys/fs/cgroup/ 1200M Y Y N \
	./compressible_data ./compressible_data /dev/urandom 120 | \
	tee "$log_dir/2-2"

echo
echo "zswap enabled with keeping incompressible pages, 20 % memory pressure"
sudo ./run.sh ../masim /sys/fs/cgroup/ 1200M Y Y Y \
	./compressible_data ./compressible_data /dev/urandom 120 | \
	tee "$log_dir/2-3"
